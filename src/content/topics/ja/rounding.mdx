...
AWS のメーターは、実際の量をそのまま読まない。**必ずどこかの単位に量子化された値を読み、丸めは常に切り上げ**だ。このサイトが索引している 185 サービスのうち 26 が、最低課金・下限・切り上げのいずれかを課金ルールに持っている。そしてそのすべてが、各サービスの料金ページの注記として、バラバラに書かれている。注記ではない。全サービスに共通する 1 本のルールで、小さい仕事が「数量 × 単価」の予想どおりの値段にならない理由はこれだ。
